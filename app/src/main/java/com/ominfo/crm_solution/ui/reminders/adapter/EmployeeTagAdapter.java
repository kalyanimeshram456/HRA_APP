...
import com.ominfo.crm_solution.ui.reminders.model.EmployeeListResult;
...
public class EmployeeTagAdapter extends RecyclerView.Adapter<EmployeeTagAdapter.ViewHolder> {
    ListItemSelectListener listItemSelectListener;
    private List<RmListModel> mListData;
    private Context mContext;
    private String mDate;
    private int valStatus;
    List<EmployeeListResult> RMDropdown = new ArrayList<>();
    public EmployeeTagAdapter(Context mContext) {
        this.mContext = mContext;
    }

    public EmployeeTagAdapter(Context context, List<RmListModel> listData, ListItemSelectListener itemClickListener) {
   ...
   /* @Override
    ...
    public void updateRmList(List<EmployeeListResult> RMDropdown){
    ...
    private void setDropdownRM(int position,List<EmployeeListResult> RMDropdown,AppCompatAutoCompleteTextView tvTitle) {
        try {
            int pos = 0;
                ...
                tvTitle.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int pos, long id) {
                        ...
                        if(tvTitle.getText().toString().trim()!=null && !tvTitle.getText().toString().trim().equals("")) {
                            mListData.set(position, new RmListModel(tvTitle.getText().toString().trim(), "0", mListData.get(position).getId()));
                            notifyItemChanged(position);
                            mListData.add(new RmListModel("", "1", mListData.get(position).getId()));
